...
$( document ).ready(function() {
  ...
  table = $('#table').DataTable({ 
    ...
    "order": [], 
    "ajax": {
      "url": baseURL + "/admin/users/getdata",
      "type": "POST",
      "dataType": "JSON",
    ...
    "columns": [
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          ...
          return '<a href="'+ uriImages +'" target="_blank">'+
                  '<img class="img-thumbnail lazy-load-img" src="'+ loaderImg +'" data-src="'+ uriThumnails +'" style="width: 35%;object-fit: contain;" />'
            '</a>';
        }
      },
      { 
      ...
      },
      {
        ...
        "className": "dt-center details-control",
        render: function (data, type, row, meta) {
          var res = (data == '1') ? '<span class="label bg-green">Active</span>' : '<span class="label bg-red">Inactive</span>';
          return res;
        }
      },
      {
        ...
        render: function (data, type, row, meta) {
          ...
          switch(data) {
            case '1':
              res = '<span class="label bg-blue">Administrator</span>';
            break;
            case '2':
              res = '<span class="label bg-orange">Admin</span>';
            break;
            case '3':
              res = '<span class="label bg-purple">Employee</span>';
            break;
            case '4':
              res = '<span class="label bg-maroon">Owner</span>';
            break;
          }
          return res;
        }
      },   
      {
        ...
        "className": "dt-center details-control",
        render: function (data, type, row, meta) {
          var res = (data == '1') ? '<span class="label bg-green">Online</span>' : '<span class="label bg-red">Offline</span>';
          return res;
        }
      },                                           
      {
...
function formNewData() {
  var url = baseURL +'/admin/users/form';
  return window.location.href = url;
}

function formEdit(id) {
  var url = baseURL +'/admin/users/form/edit?q=' + id;
  return window.location.href = url;
}

function Delete(id) {
  var url = baseURL +'/admin/users/delete?q=' + id;
