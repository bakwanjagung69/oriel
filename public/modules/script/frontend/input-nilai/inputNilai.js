...
$( document ).ready(function() {
  ...
  tableSUP = $('#table-sup').DataTable({ 
    ...
    "order": [], 
    "ajax": {
      "url": baseURL + "/inputNilai/getdataSUP",
      "type": "POST",
      "dataType": "JSON",
    ...
    "columns": [
        ...
        render: function (data, type, row, meta) {
          ...
          switch(row.status) {
            case '0':
              if (row.userRules == '4') {
                /** Kaprodi */
                ele = '<span class="badge badge-info" style="font-size: 10px;">MENUNGGU INPUT NILAI</span>';
              }
            ...
            case '1':
              ...
              if (row.userRules == '3') {
                /** DOSEN */
                ele = '<span class="badge badge-success" style="font-size: 10px;">SELESAI</span>';
              }
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          var ele = '';
          var url = baseURL + `/inputNilai/form?q=${row.formulir_sup_id}&type=sup&inputNilaiId=${row.id}`;
          ...
          switch(row.status) {
            case '0':
              ...
              if (row.userRules == '3') {
                /** DOSEN */
                ele = '<a href="'+ url +'" class="btn btn-outline-secondary btn-sm" target="_parent">Input Nilai</a>';
              }
            ...
            case '1':
              ...
              if (row.userRules == '3') {
                /** DOSEN */
                var _urls = baseURL + `/hasilNilai`;
                ele = '<a href="'+ _urls +'" class="btn btn-outline-secondary btn-sm" target="_parent">Lihat Nilai</a>';
              }
  ...
  //   var url = baseURL + `/inputNilai/form?q=${formulir_sup_id}&type=sup&tempat=${tempat}&tanggalWaktu=${tanggal_dan_waktu}&jadwalId=${input_jadwal_id}`;
  ...
  tableSkripsi = $('#table-skripsi').DataTable({ 
    ...
    "order": [], 
    "ajax": {
      "url": baseURL + "/inputNilai/getdataSkripsi",
      "type": "POST",
      "dataType": "JSON",
      "dataSrc": function ( json ) {
        // console.log('Load Complete!');
    ...
    "columns": [
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          var ele = '';
          var url = baseURL + `/inputNilai/form?q=${row.formulir_skripsi_id}&type=skripsi&inputNilaiId=${row.id}`;
  ...
  //   var url = baseURL + `/inputNilai/form?q=${formulir_skripsi_id}&type=skripsi&tempat=${tempat}&tanggalWaktu=${tanggal_dan_waktu}&jadwalId=${input_jadwal_id}`;
  //   return window.location.href = url;
  // });
  /** [END] - DatetTable SUP */
