<?php
	...
	header("Content-Disposition: attachment; filename=Data-Jadwal-Sidang-SKRIPSI-".date("Y-m-d").".xls");
					...
					<table class="table" border="1">
						...
						<tbody>
							<?php for ($i = 0; $i < count($data); ++$i) { ?>
								...
								<td class="text-center">
									<?php foreach ($data[$i]['dospemSkripsiData'] as $item) { ?>
								...
								<td class="text-center">
									<?php foreach ($data[$i]['dospemSkripsiData'] as $item) { ?>
										<?php if ($item['type_penugasan'] == 'seketaris') { ?>
											<div><?= ucfirst($item['dosen_name']); ?></div>
										<?php } ?>
									<?php } ?>
								</td>
								...
								<td class="text-center">
									<?php foreach ($data[$i]['dospemSkripsiData'] as $item) { ?>
										<?php if ($item['type_penugasan'] == 'dosen ahli') { ?>
											<div><?= ucfirst($item['dosen_name']); ?></div>
										<?php } ?>
									<?php } ?>
								</td>
