...
class SupModel extends Model {
  ...
  protected $builder;
  protected $directoryUploaded = 'assets/upload/sup/';
  ...
  public function getDataFormSUPbyNimAndStatus($nim) {
  ...
  public function AddNew($data) {
    ...
    $validationData = $this->getDataFormSUPbyNimAndStatus($data['items']['nim']);

    if ($validationData > 0) {
      return array("code" => 302, "response" => '<div style="font-size: 15px;">Anda sebelumnya sudah pernah membuat Pengajuan SUP, harap menunggu hasil SUP anda sebelumnya dari Admin TU.</div>');
      exit();
    }
    ...
    if ($data['files']['surat_tugas_file']['name'] !== '') {
      ...
      $filesSuratTugas = explode('.', $data['files']['proposal_file']['name']);
    ...
    if ($result == 1) {
     ...
     if ($data['files']['surat_tugas_file']['name'] !== '') {
      ...
      $response = array("code" => 201, "response" => 'Pendaftaran SUP berhasil dan silahkan menunggu untuk dibuatkan jadwal sidang.');
    } else {
