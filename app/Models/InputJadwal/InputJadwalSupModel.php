...
class InputJadwalSupModel extends Model {
  ...
  public function AddNewDataKirimJadwalSidang($data) {    
    ...
    $supId = $this->classesModel->decrypter($data['items']['supId']);

    $getQueryFormSup = $this->db->table('formulir_sup');
    $resultDataSup = $getQueryFormSup->getWhere(['id' => $supId])->getRowArray();

    $getQueryUserData = $this->db->table('user');
    $resultUserData = $getQueryUserData->getWhere(['nim' => $resultDataSup['nim']])->getRowArray();

    $mhsId = $resultUserData['id'];

    $insertKirimJadwalSidangMhs = $this->db->table('input_jadwal_sup')->insert([
      'formulir_sup_id' => $this->classesModel->decrypter($__data['supId']),
      'mahasiswa_id' => $mhsId,
      'judul' => $__data['judul'],
      ...
      if ($insertKirimJadwalSidangMhs == 1) {
        ...
        SET
          ...
          `updateBy` = '{$userId}',
          `updateDate` = '{$dateNow}'
        WHERE `id` = '{$this->classesModel->decrypter($__data['supId'])}'";
        $resultUPdateStatusSemesterFormSUP = $this->db->query($sql);

        if ($resultUPdateStatusSemesterFormSUP == 1) {
          $lengthData = count($__data['dosenId']);
          for ($i=0; $i < $lengthData; $i++) { 
            $insertKirimJadwalSidang = $this->db->table('input_jadwal_sup')->insert([
              'formulir_sup_id' => $this->classesModel->decrypter($__data['supId']),
