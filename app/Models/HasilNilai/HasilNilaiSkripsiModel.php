...
class HasilNilaiSkripsiModel extends Model {
  ...
  public function getDatatables(){
    ...
    if ($this->session->user_logged['rules'] == 3) {
      ...
      if ($this->request->getPost('semester') !== '') {
        ...
        $queryParams = ['dosen_id' => $this->session->user_logged['id'], 'input_jadwal_type' => 'skripsi'];
      }

      $query = $getQuery->select('*')->getWhere($queryParams);
    } 
  ...
  public function getMahasiswaId($form_skripsi_id) {
    ...
    $resultDataFormSkripsi = $getQueryNim->getWhere(['id' => $form_skripsi_id])->getRowArray();

    $getQueryUserData = $this->db->table('user');
    $resultUserData = $getQueryUserData->getWhere(['nim' => $resultDataFormSkripsi['nim']])->getRowArray();
  ...
  public function getDataInputJadwal($ids) {
    ...
    // $resultData = $getQuery->getWhere(['id' => $inputNilaiId])->getRowArray();
    // return $resultData;
    ...
    if ($this->session->user_logged['rules'] == 2) {
      $getQuery   = $this->db->table('input_jadwal_skripsi');
      $resultData = $getQuery->getWhere(['formulir_skripsi_id' => $ids])->getRowArray();
    } 
  ...
  public function calculateTotalNilaiToGradeResult($nim) {
    ...
    $resultDataInputNilai = $getQueryInputNilai->getWhere(['mahasiswa_id' => $mhsId, 'input_jadwal_type' => 'skripsi'])->getResult();
    ...
    foreach ($resultDataInputNilai as $items) {
      ...
      $resFromSkripsiId = $getFormSkripsiId->getWhere(['id' => $items->input_jadwal_id])->getRowArray();

      $getQueryTotalDosen = $this->db->table('input_jadwal_skripsi')->select('COUNT(`formulir_skripsi_id`) AS total_dosen_input');
      $resultTotalInputDosen = $getQueryTotalDosen->getWhere(['formulir_skripsi_id' => $resFromSkripsiId['formulir_skripsi_id']])->getRowArray();
